...
# # res = requests.get("https://uno.ma/iphone-maroc/iphone-7-maroc/")
res = requests.get("https://uno.ma/iphone-maroc/iphone-7-maroc/?limit=100")
html = res.text
    ...
    for num, ic in enumerate(items_cards):
        ...
        try:
            ...
            if(len(myresult) == 0):
                ...
                val = ("iPhone 7",item_title, item_link, 1, 1, item_color,item_stockage, "", image_item, item_price,scraped_at)
                mycursor.execute(sql, val)
                print("id = {}".format(
                    mycursor.lastrowid
                ))
