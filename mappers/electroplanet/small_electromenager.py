...
from mappers.Helpers.electroplanet_helprs.generale_purposed_functions import get_electroplanet_small_electromenager, get_category_id_small_electromenager
from mappers.Helpers.electroplanet import extract_specification_json, get_brand_id

def small_electromenager(brands : List) -> List:
    #Getting Electroplanet Products
    results = get_electroplanet_small_electromenager()
    res_to_post_fastapi = []
    ...
    for r in results:
        ...
        tmp_d["category_in_store_to_id"] = get_category_id_small_electromenager(r["category_in_store"])
        tmp_d["link"] = r["link"]
