...
from mappers.Helpers.electroplanet_helprs.generale_purposed_functions import get_electroplanet_maison_et_cuisine, get_category_id_maison_et_cuisine
from mappers.Helpers.electroplanet import extract_specification_json, get_brand_id

def maison_et_cuisine(brands : List) -> List:
    #Getting Electroplanet Products
    results = get_electroplanet_maison_et_cuisine()
    res_to_post_fastapi = []
    ...
    for r in results:
        ...
        tmp_d["category_in_store_to_id"] = get_category_id_maison_et_cuisine(r["category_in_store"])
        tmp_d["link"] = r["link"]
