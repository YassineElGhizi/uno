...
from mappers.Helpers.electroplanet_helprs.generale_purposed_functions import get_electroplanet_appareil_photo, get_category_id_appareil_photo
from mappers.Helpers.electroplanet import extract_specification_json, get_brand_id

def appareil_photo(brands : List) -> List:

    results = get_electroplanet_appareil_photo();
    res_to_post_fastapi = []
    ...
    for r in results:
        ...
        tmp_d["category_in_store_to_id"] = get_category_id_appareil_photo(r["category_in_store"])
        tmp_d["link"] = r["link"]
